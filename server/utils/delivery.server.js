...
/****        FilePackage    *****/
/********************************/
function FilePackage(file,pubSub,receiving,buff){
  console.log('Delivery: FilePackage constructor() entered');
  this.name = file.name;
...
Delivery.prototype.subscribe = function() {
  console.log('Delivery: subscribe()');
  var _this = this;
...
Delivery.prototype.on = function(evt,fn) {
  console.log(`Delivery: on(${evt})`);
  this.pubSub.subscribe(evt,fn);
};

Delivery.prototype.send = function(file, buff){
  console.log('Delivery: send() entered');
  var filePackage = new FilePackage(file,this.pubSub,false, buff);
